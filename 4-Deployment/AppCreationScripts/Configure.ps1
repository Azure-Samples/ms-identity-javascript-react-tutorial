...
#> 
   ...
   Write-Host "Creating the AAD application (msal-react-static)"
   # create the application 
   $spaAadApplication = New-AzureADApplication -DisplayName "msal-react-static" `
                                               -HomePage "http://localhost:3000/" `
                                               -ReplyUrls "http://localhost:3000/" `
                                               -IdentifierUris "https://$tenantName/msal-react-static" `
                                               -PublicClient $False
   ...
   Write-Host "Done creating the spa application (msal-react-static)"
   ...
   Add-Content -Value "<tr><td>spa</td><td>$currentAppId</td><td><a href='$spaPortalUrl'>msal-react-static</a></td></tr>" -Path createdApps.html
   ...
   # Update config file for 'spa'
   $configFile = $pwd.Path + "\..\App\api\local.settings.json"
   Write-Host "Updating the sample code ($configFile)"
   $dictionary = @{ "CLIENT_ID" = $spaAadApplication.AppId;"TENANT_INFO" = $tenantId };
