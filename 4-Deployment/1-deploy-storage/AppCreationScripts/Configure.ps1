...
#> 
   ...
   Write-Host "Creating the AAD application (ms-identity-react-c4s1-api)"
   # create the application 
   $serviceAadApplication = New-AzureADApplication -DisplayName "ms-identity-react-c4s1-api" `
    ...
    if ($scopes.Count -ge 0) 
    {
        ...
        else 
        {
            ...
            $scope = CreateScope -value "access_as_user"  `
                -userConsentDisplayName "Access ms-identity-react-c4s1-api"  `
                -userConsentDescription "Allow the application to access ms-identity-react-c4s1-api on your behalf."  `
                -adminConsentDisplayName "Access ms-identity-react-c4s1-api"  `
   ...
   Write-Host "Done creating the service application (ms-identity-react-c4s1-api)"
   ...
   Add-Content -Value "<tr><td>service</td><td>$currentAppId</td><td><a href='$servicePortalUrl'>ms-identity-react-c4s1-api</a></td></tr>" -Path createdApps.html


   # Create the spa AAD application
   Write-Host "Creating the AAD application (ms-identity-react-c4s1-spa)"
   # create the application 
   $spaAadApplication = New-AzureADApplication -DisplayName "ms-identity-react-c4s1-spa" `
                                               -HomePage "http://localhost:3000/" `
                                               -ReplyUrls "http://localhost:3000/" `
                                               -IdentifierUris "https://$tenantName/ms-identity-react-c4s1-spa" `
                                               -PublicClient $False
   ...
   Write-Host "Done creating the spa application (ms-identity-react-c4s1-spa)"
   ...
   Add-Content -Value "<tr><td>spa</td><td>$currentAppId</td><td><a href='$spaPortalUrl'>ms-identity-react-c4s1-spa</a></td></tr>" -Path createdApps.html
   ...
   $requiredPermissions = GetRequiredPermissions -applicationDisplayName "ms-identity-react-c4s1-api" `
   ...
   $dictionary = @{ "Enter_the_Application_Id_Here" = $spaAadApplication.AppId;"Enter_the_Tenant_Info_Here" = $tenantId;"Enter_the_Web_Api_Scope_Here" = ("api://"+$serviceAadApplication.AppId+"/access_as_user") };
