...
name: React single-page application using MSAL React to sign-in users against Azure Active Directory
urlFragment: ms-identity-javascript-react-tutorial
description: "This sample demonstrates a React SPA that authenticates users against Azure AD"
...
This sample demonstrates a React SPA that authenticates users against Azure AD.

## Scenario

1. The client React SPA uses the Microsoft Authentication Library (MSAL) to obtain an ID Token from **Azure AD**.
...
### Register the spa app (ms-identity-react-c1s1)
...
#### Configure the spa app (ms-identity-react-c1s1) to use your app registration
...
#### Build and upload (ms-identity-react-c1s1) to an Azure Storage blob
...
1. In the `ms-identity-react-c1s1` project source code, update your configuration file with the **Primary endpoint field** as your new **Redirect URI** (you will register this URI later).
...
##### Update the Azure AD app registration for ms-identity-react-c1s1

1. Navigate back to to the [Azure portal](https://portal.azure.com).
1. In the left-hand navigation pane, select the **Azure Active Directory** service, and then select **App registrations**.
1. In the resulting screen, select `ms-identity-react-c1s1`.
1. In the app's registration screen, select **Authentication** in the menu.
   - In the **Redirect URIs** section, update the reply URLs to match the site URL of your Azure deployment. For example:
      - `https://ms-identity-react-c1s1.azurewebsites.net/`
