...
Function Cleanup
{
    ...
    Get-AzureADApplication -Filter "DisplayName eq 'msal-react-downstream'"  | ForEach-Object {Remove-AzureADApplication -ObjectId $_.ObjectId }
    $apps = Get-AzureADApplication -Filter "DisplayName eq 'msal-react-downstream'"
    if ($apps)
    {
    ...
    Get-AzureADServicePrincipal -filter "DisplayName eq 'msal-react-downstream'" | ForEach-Object {Remove-AzureADServicePrincipal -ObjectId $_.Id -Confirm:$false}
    
    Write-Host "Removing 'MiddletierAPI' (msal-react-middletier) if needed"
    Get-AzureADApplication -Filter "DisplayName eq 'msal-react-middletier'"  | ForEach-Object {Remove-AzureADApplication -ObjectId $_.ObjectId }
    $apps = Get-AzureADApplication -Filter "DisplayName eq 'msal-react-middletier'"
    if ($apps)
    {
        Remove-AzureADApplication -ObjectId $apps.ObjectId
    }
    ...
    # also remove service principals of this app
    Get-AzureADServicePrincipal -filter "DisplayName eq 'msal-react-middletier'" | ForEach-Object {Remove-AzureADServicePrincipal -ObjectId $_.Id -Confirm:$false}
