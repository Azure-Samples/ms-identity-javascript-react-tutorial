...
Role based access control in Microsoft Entra ID can be done with **Delegated** and **App** permissions and **App Roles** as well. We cover RBAC using App Roles in the [previous tutorial](../1-call-api-roles/README.md). **Delegated** and **App** permissions, **Security Groups** and **App Roles** in Microsoft Entra ID are by no means mutually exclusive - they can be used in tandem to provide even finer grained access control.
...
1. The app then obtains an [access token](https://docs.microsoft.com/azure/active-directory/develop/access-tokens) from Microsoft Entra ID on behalf of the authenticated user for the **TodoListAPI**.
...
1. Find the key `Enter_the_Application_Id_Here` and replace the existing value with the application ID (clientId) of `msal-react-app` app copied from the Microsoft admin center.
1. Find the key `Enter_the_Client_Secret_Here` and replace the existing value with the key you saved during the creation of `msal-react-app` copied from the Microsoft admin center.

1. Open the `SPA\src\authConfig.js` file.
1. Find the key `Enter_the_Application_Id_Here` and replace the existing value with the application ID (clientId) of `msal-react-app` app copied from the Microsoft admin center.
1. Find the key `Enter_the_Tenant_Info_Here` and replace the existing value with your Microsoft Entra tenant/directory ID.
1. Find the key `Enter_the_Web_Api_Application_Id_Here` and replace the existing value with the application ID (clientId) of `msal-react-app` app copied from the Microsoft admin center.
...
1. Open the `SPA\src\authConfig.js` file.
2. Find the key `Enter_the_Object_Id_of_GroupAdmin_Group_Here` and replace the existing value with the **object ID** of the **GroupAdmin** group copied from the Microsoft admin center.
3. Find the key `Enter_the_Object_Id_of_GroupMember_Group_Here` and replace the existing value with the **object ID** of the **GroupMember** group copied from the Microsoft admin center.

4. Open the `API\authConfig.json` file.
5. Find the key `Enter_the_Object_Id_of_GroupAdmin_Group_Here` and replace the existing value with the **object ID** of the **GroupAdmin** group copied from the Microsoft admin center.
6. Find the key `Enter_the_Object_Id_of_GroupMember_Group_Here` and replace the existing value with the **object ID** of the **GroupMember** group copied from the Microsoft admin center.
